...
bool dualDeployEvents = true; // whether or not dual deployment events will occur; false means only primary deployment in pitch config
...
int main(){
	...
	// do first deployment test (pitch config)
	...
	std::cout << "first deploy" << std::endl;
	
	sleep(5);
	
	// do second deployment test (drag config)
	
    moveServoPair(servoPinN, servoPinS, airfoilTiltAngle, !pitchFirst);
    moveServoPair(servoPinE, servoPinW, -airfoilTiltAngle, !pitchFirst);
		
	std::cout << "second deploy" << std::endl;
	
		sleep(3);
	
	// for correct drag deployment, one airfoilTiltAngle is opposite of the other and bool is set to false
